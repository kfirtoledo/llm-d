...
| **Remote / shared KV tier (cluster-wide, L3)** | vLLM: **OffloadingConnector** (± Mooncake store). SGLang: **HiCache L3 = Mooncake** (`--hicache-storage-backend mooncake`) | 🔸 engine-native, **not wired in llm-d** | Cluster-shared, pull-on-miss KV store across pods. Same capability, different mechanism. llm-d ships the **CPU/L2 tier only** today → Mooncake L3 tracked in **[#1980](https://github.com/llm-d/llm-d/issues/1980)** |
| **P2P KV pull (cross-instance / prefix pull)** | vLLM: `offloading` connector + `--enable-p2p-pull` (`x-kv-cache-source-host-port`), decode/instance pulls a cached prefix from another instance. SGLang: **Mooncake store** as the shared pull tier (`--hicache-storage-backend mooncake`) | ❌ (both) | **Not fully implemented on the vLLM path yet — open issue.** Distinct from P/D transport (which is a *push* prefill→decode). For SGLang the pull tier is engine-capable via Mooncake but **not wired in llm-d**; overlaps with the storage-offload L3 work ([#1980](https://github.com/llm-d/llm-d/issues/1980)) |
...
| **tiered-prefix-cache** | HBM→CPU KV offload | ✅ **native (HiCache)** | `modelserver/gpu/sglang/native/cpu/`; README:23,28,181; benchmark shipped |
...
| HiCache offload | ✅ Yes | `--hicache-size/-write-policy/-io-backend/-storage-backend {file,mooncake,nixl,hf3fs,...}` | ✅ CPU RAM tier only |
...
4. **Storage-offload / shared remote KV tier** — *capability exists for SGLang, not yet wired.* vLLM uses the **OffloadingConnector**; SGLang's equivalent is **HiCache L3/L4 backed by a distributed store** (`--hicache-storage-backend {mooncake, lmcache, lustre, hf3fs, …}`) — a cluster-shared, pull-on-miss tier, engine-native. Different mechanism, same capability (**not** "SGLang has no P2P tier"). llm-d wires only the CPU **L1/L2** HiCache tier today → L3/L4 tracked in **[#1980](https://github.com/llm-d/llm-d/issues/1980)** (mooncake) + **[#1967](https://github.com/llm-d/llm-d/issues/1967)** (lustre) (+ router [llm-d-router#1010](https://github.com/llm-d/llm-d-router/issues/1010)). Separately, the llm-d `mooncake` **P/D** connector is still vLLM-`kv_transfer_params`-only.
...
| P/D disaggregation — **RDMA recipes** | 🔄 In progress | 0.9 | Rahul; docs [#1641](https://github.com/llm-d/llm-d/issues/1641) |
...
| Tiered prefix cache **L3/L4** (lmcache/lustre/mooncake) | ⬜ Not started | — | [#1980](https://github.com/llm-d/llm-d/issues/1980), [#1967](https://github.com/llm-d/llm-d/issues/1967); router [#1010](https://github.com/llm-d/llm-d-router/issues/1010) |
...
| Mooncake as **P/D connector** | 🔄 Partial | 0.9 | [#1524](https://github.com/llm-d/llm-d/issues/1524) (#1556 merged) |
...
SGLang is a **first-class backend** for the four core well-lit paths (optimized-baseline, precise-prefix, P/D on NVIDIA, tiered HiCache) and inherits all engine-agnostic routing. The remaining gaps — **LoRA routing (no tracking issue), multimodal/encode-disagg (no tracking issue), wide-EP large-MoE ([#2040](https://github.com/llm-d/llm-d/issues/2040)), shared remote KV tier via HiCache L3/L4 ([#1980](https://github.com/llm-d/llm-d/issues/1980), [#1967](https://github.com/llm-d/llm-d/issues/1967)), P2P KV pull (no tracked issue), AMD P/D, CI coverage ([#2045](https://github.com/llm-d/llm-d/issues/2045))** — are **all llm-d wiring gaps (missing overlays / metric mappings), not SGLang engine limitations**. Key nuance: SGLang can't use vLLM's P/D or offloading *connectors*, but reaches the **same shared/remote KV tier via its own HiCache L3/L4 store** — different mechanism, not a missing feature. **§6b** is the full tracking + roadmap table (issues, status, target release) — 0.8 shipped; 0.9/1.0 in flight.
